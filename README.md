...
│   └── pages/                  # Páginas e módulos de interface
├── core/
...
└── ...
```
> A organização exata pode evoluir ao longo do projeto. Ao adicionar módulos, mantenha a separação entre interface (`app/`), regras analíticas/dados (`core/`) e testes (`tests/`).
---
🛠️ Tecnologias
...
```
A cobertura é medida prioritariamente no diretório:
```text
core/
```
A meta mínima configurada no projeto é de 80% de cobertura. O pipeline de integração contínua deve falhar caso a cobertura fique abaixo desse limite.
...
---
<p align="center">
  <strong>Argos DataLab</strong><br />
