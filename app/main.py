...
if load_data:
    ...
    with st.spinner("Baixando e processando dados..."):
        ...
        # -----------------------------------------------------------------
        # CÁLCULO DA VOLATILIDADE MENSAL ANUALIZADA (Requisito Matemático)
        # -----------------------------------------------------------------
        # O cálculo precisa ser feito com dados diários antes da agregação principal
        df_vol = df_prepared[['Date', 'Close']].copy()
        df_vol['Date'] = pd.to_datetime(df_vol['Date'])
        
        # 1. Cálculo dos Retornos Diários (fechamento atual / fechamento anterior - 1)
        df_vol['Daily_Return'] = df_vol['Close'].pct_change()
        
        # 2. Agrupamento por Mês
        df_vol['YearMonth'] = df_vol['Date'].dt.to_period('M')
        
        # 3. Desvio Padrão Amostral dos retornos diários no mês (calcula a média implicitamente)
        monthly_std = df_vol.groupby('YearMonth')['Daily_Return'].std()
        
        # 4 & 5. Anualização (sqrt(365)) e Conversão para Porcentagem (* 100)
        monthly_vol_annualized = monthly_std * np.sqrt(365) * 100
        
        # Preparando DataFrame final para o gráfico
...
if df_monthly_vol is not None and not df_monthly_vol.empty:
    # Destaca os 5% de meses mais voláteis (Outliers) para recriar o visual do modelo
    threshold = df_monthly_vol['Volatility'].quantile(0.95)
    
    # Aplica cor amarela aos picos, VERDE ao restante (alinhado com os outros gráficos)
